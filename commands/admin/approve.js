...
module.exports = (bot) => {
  ...
  bot.onText(/\/approve (\d+)/, async (msg, match) => {
    ...
    if (!match || !match[1]) {
      return bot.sendMessage(adminId, '⚠️ Usage: /approve <playerId>');
    }
    ...
    if (!room) {
      return bot.sendMessage(adminId, '❌ Player not found in any room.');
    }
    ...
    await bot.sendMessage(adminId, `🎉 Approved Bingo for ${player.username || targetId}`);
  });
  ...
  bot.onText(/\/stats/, async (msg) => {
    ...
    try {
      ...
      await bot.sendMessage(adminId, '⚠️ Failed to fetch game stats.');
    }
  });
};
